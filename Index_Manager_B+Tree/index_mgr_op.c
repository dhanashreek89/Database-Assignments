...
RC insertLeaf(Btree *root, Value* key, RID rid) {
	...
	for(idx = 0; idx < root->num_keys && root->keys[idx] < key->v.intV; idx++) { }
...
RC insert_parent(BTreeHandle* tree, Btree_stat *root, Btree *prev_node, Btree *new_node, int new_key) {
	...
	if (!prev_node->parent) {
		...
		// set the prev and next nodes
		parent_node->pointers[0] = prev_node;
		...
		// not a root
		parent_node = getParent(prev_node, new_key);
		totKeys = parent_node->num_keys;

		parent_node->pointers[totKeys + 1] = new_node;
	}
	parent_node->is_leaf = false;

	prev_node->parent = parent_node;
...
// split the node if needed while insertion
RC splitInsert(BTreeHandle* tree, Btree_stat *root, Btree *oldNode, 
        Value* key, RID rid) { 
    
	int index, i, j, num_newNode, new_key, split_pos;
	i =0;
	j =0;
	index = 0;
	new_key = 0;
	...
	int temp_array_keys[10000];
        RID temp_array_pointers[10000];
    	Btree *newNode, *temp; 
	...
	Btree_stat* stat = tree->mgmtData; 
		...
		for (index=0; index < oldNode->num_keys && oldNode->keys[index] < key->v.intV; index++) {}
